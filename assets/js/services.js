...
/**
 * 
 * define services by Julienedies
 * 
 */
...
(function(angular){
	...
	bmServices.factory('e', ['$q','$rootScope', function($q, $rootScope) {
	...
	bmServices.factory('cStorageInterface', ['$q', '$rootScope', function($q, $rootScope) {
		...
		for(var j in events){
			...
			var callback = (function(j){
				...
				return function(){
					var args = Array.prototype.slice.call(arguments, 0); 
					console.log('chrome.storage.sync.change',j);
					args.unshift(j);
		...
		for(var i in fns){
			...
			fn.prototype[i] = (function(i){
				...
				return function(){
					...
					//数据发生变化时当前页面是监听不到storage事件变更消息的，所以只能手动触发
					if(/set|remove|clear/.test(i)){
						console.log(that.classify + '.recordManager.change');
						$rootScope.$broadcast(that.classify + '.recordManager.change',args.unshift(i));
					}	
